...
Light light;
GLfloat camPosition[] = { 0, 0, 0, 0, 0,-1, 0, 1, 0 };
...
void drawModel() {
	...
	GLfloat color[2][3] = { { 0.6,0.6,0.6 },{ 0.6,0.6,0.6 } };
	light.setAmbientMaterial(color[0]);
	light.setDiffuseMaterial(color[1]);
	...
	glBegin(GL_TRIANGLES);
	{
		for (int i = 0; i < faceNum; i++) {
			...
			for (int j = 0; j < 3; j++) {
				...
				GLfloat vertex[3], normal[3];
				vertex[0] = tempR[0] * tempFaceVertex[j].x + tempR[4] * tempFaceVertex[j].y + tempR[8] * tempFaceVertex[j].z + tempR[12];
				vertex[1] = tempR[1] * tempFaceVertex[j].x + tempR[5] * tempFaceVertex[j].y + tempR[9] * tempFaceVertex[j].z + tempR[13];
				vertex[2] = tempR[2] * tempFaceVertex[j].x + tempR[6] * tempFaceVertex[j].y + tempR[10] * tempFaceVertex[j].z + tempR[14];
				normal[0] = tempR[0] * tempFaceNormal[j].x + tempR[4] * tempFaceNormal[j].y + tempR[8] * tempFaceNormal[j].z;
				normal[1] = tempR[1] * tempFaceNormal[j].x + tempR[5] * tempFaceNormal[j].y + tempR[9] * tempFaceNormal[j].z;
				normal[2] = tempR[2] * tempFaceNormal[j].x + tempR[6] * tempFaceNormal[j].y + tempR[10] * tempFaceNormal[j].z;
				light.setLightColor(camPosition, vertex, normal);
